...
5. 如果是 table route：
   ...
   - 同時透過 `src/vectorstores/chroma_store.py` 從 Chroma 補充檢索相關 chunks
6. 如果是 vector route：
...
| Context Precision | 0.938 |
| Context Recall | 1.000 |
| Answer Correctness | 0.739 |


RAGAS 評估顯示，retrieval 階段已具備穩定表現。原始問句與模擬真實使用者問法在 retrieval evaluation 中皆能於 Top-3 內命中正確文件，因此目前暫不導入 reranker。主要原因是目前資料集規模較小、查詢範圍明確，加入 reranker 可能帶來額外延遲、成本與系統複雜度，但實際收益有限。

在 answer evaluation 中，初版 `answer_correctness` 分數相對較低，初步判斷原因包括：

- prompt 讓模型傾向補充較多說明，導致回答與 ground truth 的形式不完全一致
- ground truth 來自原始 QA 資料，部分答案較精簡，未必完整涵蓋可接受回答範圍
- table route 若只使用表格資料，可能錯過向量資料庫中的相關 QA context

後續經過兩項調整後，RAGAS 指標已有改善：

- 收斂 prompt，限制補充重點必須由 retrieved context 明確支持
- 將 table route 調整為 table primary + vector support，讓表格查詢同時保留向量檢索 context

調整後，`faithfulness` 與 `answer_correctness` 均有提升。人工檢視部分低分案例後，發現模型回答多數仍屬於合理補充；部分 `answer_correctness` 誤差主要來自 ground truth 較精簡，導致完整但語意延伸較多的回答被低估。
...
此頁面可載入 `eval/results/` 下的 JSON 結果檔，並以儀表板形式顯示 summary、各項指標、逐題分數、模型回答與 ground truth，也支援不同結果檔之間的版本比較。

若要使用下拉選單直接載入結果檔，可在專案根目錄啟動簡單靜態伺服器：
...
接著開啟：
```text
http://localhost:8000/eval/results_viewer.html
```
## API 與前端
### 後端
後端入口為：
...
- 擴充 realistic / stress query 評估集，增加更短、更模糊或帶有錯字的使用者查詢
